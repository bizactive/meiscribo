...
//set upward height once bounced? - not sure what this does
let grav = 0.99;
...
this.radius = Math.random() * 20 + 44;  
this.startradius = this.radius;
this.x = Math.random() * (winInWid - this.radius * 2) + this.radius; 
this.y = Math.random() * (winInHgt - this.radius); 
this.dx = Math.round((Math.random() - 0.5) * 15);  
this.dy = Math.random() * 2; //used in animation for?
this.vel = Math.random() / 5;  
  ...
  this.isClicked = function(clickX, clickY) { // Calculate the distance from the click to the center of the ball
    const distance = getDistance(clickX, clickY, this.x, this.y);  // If the click is within the radius of the ball, change its color to red
...
//array of Balls
let ballz = [];
for (let i = 0; i < 15; i++) { //number of Balls
  ballz.push(new Ball())
;}
...
if (mousex > ballz[i].x - 20 &&
  ...
  ballz[i].radius < 70) {
    ...
    ballz[i].radius += 5;} //increase and decrease had no effect - test
// else if?
else { 
